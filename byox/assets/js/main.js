/* ============================================================
   BYOX — main.js : renders the index from tutorials.js data
   ============================================================ */
...
const CATEGORY_ICONS = {
  ...
  "واقعیت افزوده": "🥽",
  "کلاینت بیتتورنت": "🧲",
  "بلاکچین / ارز دیجیتال": "⛓️",
  "ربات": "🤖",
  "ابزار خط فرمان": "⌨️",
  "پایگاه داده": "🗄️",
  "داکر": "🐳",
  "شبیهساز / ماشین مجازی": "🧮",
  "فریمورک / کتابخانه فرانتاند": "🧩",
  "بازی": "🕹️",
  "گیت": "🌿",
  "تخصیصدهنده حافظه": "🧠",
  "پشته شبکه": "🌐",
...
function init() {
  ...
  if (fill && data.length) {
    ...
    if (label) label.textContent = "پیشرفت ترجمه: " + pct + "٪ (" + done + " از " + data.length + " آموزش ترجمه شده)";
  }
...
function render(data) {
  ...
  let showed = 0;
  order.forEach((cat) => {
    const items = cats[cat].filter(matches).sort((a, b) => b.done - a.done);
    ...
    head.appendChild(h2);
    head.appendChild(el("span", "meta", "ترجمهشده: " + done + " از " + items.length));
  ...
  if (!showed) {
    const note = el("div", "empty-note", "چیزی پیدا نشد — عبارت دیگری را جستوجو کنید.");
    root.appendChild(note);
  }
}

function card(t) {
  const c = el("a", "card" + (t.done ? "" : " wait"));
  if (t.done) c.href = t.file;
  else c.setAttribute("aria-disabled", "true");

  const top = el("div", "top");
  top.appendChild(el("span", "lang", t.lang));
  if (t.video) top.appendChild(el("span", "badge-status video", "ویدئو"));
  c.appendChild(top);

  c.appendChild(el("div", "title", t.title));

  const bottom = el("div", "bottom");
  bottom.appendChild(el("span", "badge-status " + (t.done ? "done" : "wait"), t.done ? "ترجمه شده" : "بهزودی"));
  c.appendChild(bottom);

  const go = el("span", "go", t.done ? "خواندن آموزش ←" : "");
